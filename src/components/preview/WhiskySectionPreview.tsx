...
// Define WhiskySection interface (likely same as CharitySection)
interface WhiskySection {
  id: string
  section_key: string
  content: string
  order_number: number
  style_type: string // 'title', 'paragraph', 'link', etc.
}

interface WhiskySectionPreviewProps {
  sections: WhiskySection[]
}
...
export function WhiskySectionPreview({ sections }: WhiskySectionPreviewProps) {
  const containerRef = useRef<HTMLDivElement>(null)
  const [visibleSections, setVisibleSections] = useState<WhiskySection[]>([])
